<nav class="flex flex-col flex-1 p-3 space-y-1">
    {{-- LINK UTAMA --}}
    <a href="{{ Auth::user()->role == 'admin' ? route('admin.dashboard.index') : route('officer.dashboard.index') }}"
        ...
        <span x-show="sidebarExpanded" class="sidebar-content" x-cloak>@lang('system.nav_dashboard')</span>
    </a>

    {{-- GRUP UNTUK OFFICER & ADMIN --}}
    @if (in_array(Auth::user()->role, ['officer', 'admin']))
        <div x-show="sidebarExpanded"
            class="pt-4 pb-1 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider sidebar-content"
            x-cloak>@lang('system.nav_group_operational')</div>
        ...
        <a href="{{ route('officer.complaints.index') }}"
            ...
            <span x-show="sidebarExpanded" class="sidebar-content" x-cloak>@lang('system.nav_complaints')</span>
        </a>
        <a href="{{ route('officer.schedules.index') }}"
            ...
            <span x-show="sidebarExpanded" class="sidebar-content" x-cloak>@lang('system.nav_schedules')</span>
        </a>
        <a href="{{ route('officer.informations.index') }}"
            ...
            <span x-show="sidebarExpanded" class="sidebar-content" x-cloak>@lang('system.nav_educations')</span>
        </a>

        <div x-show="sidebarExpanded"
            class="pt-4 pb-1 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider sidebar-content"
            x-cloak>@lang('system.nav_group_master_data')</div>
        <a href="{{ route('officer.locations.index') }}"
            ...
            <span x-show="sidebarExpanded" class="sidebar-content" x-cloak>@lang('system.nav_locations')</span>
        </a>
        <a href="{{ route('officer.bins.index') }}"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0"
                ...
                <path d="M6 3a2 2 0 012-2h4a2 2 0 012 2v2a2 2 0 01-2 2H8a2 2 0 01-2-2V3z" />
                <path d="M7 7h6v11a1 1 0 01-1 1H8a1 1 0 01-1-1V7z" />
            </svg>
            <span x-show="sidebarExpanded" class="sidebar-content" x-cloak>@lang('system.nav_bins')</span>
        </a>
    @endif
    ...
    @if (Auth::user()->role == 'admin')
        <div x-show="sidebarExpanded"
            class="pt-4 pb-1 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider sidebar-content"
            x-cloak>@lang('system.nav_group_system')</div>
        <a href="{{ route('admin.users.index') }}"
            ...
            <span x-show="sidebarExpanded" class="sidebar-content" x-cloak>@lang('system.nav_users')</span>
        </a>
    @endif
</nav>
