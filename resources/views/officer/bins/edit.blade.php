...
<x-layout :breadcrumb="$breadcrumb">
    ...
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-semibold text-gray-900 mb-6">Edit Tempat Sampah
        </h1>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <form action="{{ route('officer.bins.update', $bin->id) }}" method="POST"
                ...
                <div class="border-t border-gray-200 pt-6">
                    <label class="block text-sm font-medium text-gray-700">QR
                        Token</label>
                    <div class="mt-1 p-3 bg-gray-100 rounded-md text-gray-600 font-mono text-sm">
                        {{ $bin->qr_token }}
                    </div>
                    <p class="mt-1 text-xs text-gray-500">Token ini dibuat
                        otomatis dan tidak dapat diubah.</p>
                </div>

                <div class="flex items-center gap-4 pt-4">
                    <button type="submit"
                        class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium">Perbarui</button>
