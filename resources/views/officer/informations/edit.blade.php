        ...
        <form action="{{ route('officer.informations.update', $information->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white shadow-md rounded-lg p-6">
                        <label for="content"
                            class="block text-sm font-medium text-gray-700">Konten</label>
                ...
                <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select id="category" name="category" required
                                    ...
                                    <option value="Edukasi" @selected(old('category', $information->category) == 'Edukasi')>Edukasi
                                    </option>
                                    <option value="Berita" @selected(old('category', $information->category) == 'Berita')>Berita
                                    </option>
                                    <option value="Pengumuman" @selected(old('category', $information->category) == 'Pengumuman')>
                                        Pengumuman</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="status" name="status" required
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500">
                                    <option value="published" @selected(old('status', $information->status) == 'published')>Published
                                    </option>
                                    <option value="draft" @selected(old('status', $information->status) == 'draft')>Draft
                                    </option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="bg-white shadow-md rounded-lg p-6">
                        ...
                        <p class="text-xs text-gray-500 mt-1">Biarkan kosong untuk mempertahankan
                            gambar saat ini.</p>
                    </div>
                </div>
            </div>
            <div class="mt-8 flex items-center gap-4"><button type="submit"
                    ...
                    Artikel</button><a href="{{ route('officer.informations.index') }}"
