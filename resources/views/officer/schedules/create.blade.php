@php
    $breadcrumb = [
        ...
        ['label' => __('system.breadcrumb_add_schedule'), 'url' => '#'],
    ];
@endphp

<x-layout :breadcrumb="$breadcrumb">
    <x-slot:page_title>
        @lang('system.schedules_create_title')
    </x-slot>
            ...
            return {
                ...
                toggleAll(event) {
                    if (event.target.checked) {
                        ...
                        // Hapus centang hanya untuk yang terlihat
                        let visibleLocationIds = [];
                        document.querySelectorAll('.location-checkbox-item').forEach(el => {
                            if (el.style.display !== 'none') {
                                visibleLocationIds.push(parseInt(el.querySelector('input')
                        ...
                        this.selectedLocations = this.selectedLocations.filter(id => !visibleLocationIds
                            .includes(id));
                    }
                }
            }
    ...
    <div class="max-w-4xl mx-auto">
        <h1 class="text-2xl font-semibold text-gray-900 mb-6">@lang('system.schedules_create_title')</h1>
        ...
        <form action="{{ route('officer.schedules.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="day" name="day" required
                                    ...
                                    <option value="Monday" @selected(old('day') == 'Monday')>
                                        @lang('public.day_monday')</option>
                                    <option value="Tuesday" @selected(old('day') == 'Tuesday')>
                                        @lang('public.day_tuesday')</option>
                                    <option value="Wednesday" @selected(old('day') == 'Wednesday')>
                                        @lang('public.day_wednesday')</option>
                                    <option value="Thursday" @selected(old('day') == 'Thursday')>
                                        @lang('public.day_thursday')</option>
                                    <option value="Friday" @selected(old('day') == 'Friday')>
                                        @lang('public.day_friday')</option>
                                    <option value="Saturday" @selected(old('day') == 'Saturday')>
                                        @lang('public.day_saturday')</option>
                                    <option value="Sunday" @selected(old('day') == 'Sunday')>
                        ...
                        @forelse($locations as $location)
                            <div class="location-checkbox-item flex items-center p-2 rounded-md hover:bg-gray-100"
                                ...
                                <input type="checkbox" id="location_{{ $location->id }}"
                                    ...
                                    x-model="selectedLocations" @checked(is_array(old('locations')) && in_array($location->id, old('locations')))
            ...
            <div class="mt-8 flex items-center gap-4">
                <button type="submit"
                    class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 font-medium transition-colors">@lang('system.btn_save_schedule')</button>
