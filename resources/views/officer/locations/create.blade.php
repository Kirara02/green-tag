@php
    $breadcrumb = [
        ...
        ['label' => __('system.breadcrumb_add_location'), 'url' => '#'],
    ];
@endphp

<x-layout :breadcrumb="$breadcrumb">
    <x-slot:page_title>
        @lang('system.locations_create_title')
    </x-slot>

    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-semibold text-gray-900 mb-6">@lang('system.locations_create_title')</h1>
            ...
            <form action="{{ route('officer.locations.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code') }}"
                        required placeholder="@lang('system.placeholder_location_code')"
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}"
                        required placeholder="@lang('system.placeholder_location_name')"
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500">
                </div>
                <div>
                    <label for="address"
                        class="block text-sm font-medium text-gray-700">@lang('system.form_full_address')</label>
                    <textarea id="address" name="address" rows="4" required placeholder="@lang('system.placeholder_full_address')"
                ...
                <div class="flex items-center gap-4 pt-4">
                    <button type="submit"
                        class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 font-medium transition-colors">@lang('system.btn_save_location')</button>
