            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white shadow-md rounded-lg">
                    ...
                    <div class="p-6 space-y-6">
                        <div><label class="text-sm font-medium text-gray-500">Kategori</label>
                            <p class="mt-1 text-base text-gray-900 font-semibold">
                                {{ ucwords(str_replace('_', ' ', $complaint->category)) }}</p>
                        </div>
                        <div><label class="text-sm font-medium text-gray-500">Lokasi</label>
                            <p class="mt-1 text-base text-gray-900">
                                {{ $complaint->bin->location->name }}</p>
                        </div>
                        <div><label class="text-sm font-medium text-gray-500">Kode Tempat
                                Sampah</label>
                            <p class="mt-1 text-base text-gray-900 font-mono">
                                {{ $complaint->bin->code }}</p>
                        </div>
                        <div><label class="text-sm font-medium text-gray-500">Deskripsi</label>
                            <p class="mt-1 text-base text-gray-900 whitespace-pre-wrap">
                                {{ $complaint->description }}</p>
                        </div>
                        @if ($complaint->photo)
                            <div>
                                <label class="text-sm font-medium text-gray-500">Foto
                                    Terlampir</label>
                                <div class="mt-2"><a
                                        href="{{ asset('storage/' . $complaint->photo) }}"
                                        target="_blank" class="inline-block"><img
                                            src="{{ asset('storage/' . $complaint->photo) }}"
                                            alt="Foto Pengaduan"
                                            class="w-full max-w-sm h-auto rounded-md border-2 border-gray-200 hover:border-green-500 transition"></a>
                                    <p class="text-xs text-gray-500 mt-1">Klik gambar untuk
                                        memperbesar</p>
                                </div>
                            </div>
                        @endif
                ...
                <div class="bg-white shadow-md rounded-lg">
                    ...
                    <div class="p-6 space-y-6">
                        <div><label class="text-sm font-medium text-gray-500">Nama Pelapor</label>
                            <p class="mt-1 text-base text-gray-900">{{ $complaint->reporter_name }}
                            </p>
                        </div>
                        <div><label class="text-sm font-medium text-gray-500">Telepon
                                Pelapor</label>
                            <p class="mt-1 text-base text-gray-900">
                                {{ $complaint->reporter_phone ?? 'Tidak diberikan' }}</p>
                        </div>
                        <div><label class="text-sm font-medium text-gray-500">Detail Alamat
                                Pelapor</label>
                            <p class="mt-1 text-base text-gray-900">{{ $complaint->address_detail }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-1">
                <form action="{{ route('officer.complaints.update', $complaint->id) }}"
                    ...
                    <div class="p-6">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Perbarui status dan tambahkan catatan
                            penyelesaian.</p>
                    </div>
                    <div class="p-6 border-t space-y-4">
                        <div>
                            <label for="status"
                                class="block text-sm font-medium text-gray-700">Atur Status</label>
                            <select id="status" name="status"
                                ...
                                <option value="in_progress" @selected($complaint->status == 'in_progress')>Sedang
                                    Diproses</option>
                                <option value="resolved" @selected($complaint->status == 'resolved')>Selesai
                                </option>
                                <option value="rejected" @selected($complaint->status == 'rejected')>Ditolak
                                </option>
                            </select>
                            @error('status')
                        ...
                        <div>
                            ...
                            <textarea id="resolution_notes" name="resolution_notes" rows="6"
                                placeholder="Tambahkan catatan jika status Selesai atau Ditolak..."
                    ...
                    <div
                        class="px-6 py-4 bg-gray-50 border-t rounded-b-lg flex items-center justify-between">
                        <a href="{{ route('officer.complaints.index') }}"
                            class="text-sm font-medium text-gray-600 hover:text-gray-900">&larr;
                            Kembali ke daftar</a>
                        <button type="submit"
                            class="px-5 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold text-sm">Perbarui
                            Pengaduan</button>
                    </div>
                </form>
            </div>
